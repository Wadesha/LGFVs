...
├── raw_files/                 # 【真实·公开披露】手动下载样本 PDF（上海/成都/湖州/新浪）
│   └── README.md              #   样本说明
├── shandong_reports/          # 【真实·公开披露】山东城投采集体系
│   ├── raw/<城市>/<公司>/*.pdf    #   28 市 / 100 主体 / 403 份披露 PDF
│   ├── raw_cninfo/            #   巨潮资讯源：100 JSON + 29 PDF
│   ├── collect_*.py 等        #   采集脚本
│   └── README.md              #   体系说明
└── .gitignore                 # 保护隐私/调试产物，真实公开数据入库
```

> 仓库现已包含**公开披露**的真实数据：`raw_files/`（样本 PDF）与 `shandong_reports/`（山东采集体系）。
> 二者均为发行人对外公开资料，不含任何私密 / 涉密内容；调试产物（截图 / 调试页）与私密数据仍被 `.gitignore` 排除。
...
- 仓库包含的真实 PDF 均为**发行人对外公开披露**资料（募集说明书 / 定期报告等），不含私密或涉密内容；
- 调试产物（截图、调试页）与任何访问凭证（Token / 密钥）**一律不入库**；
